...
const CountrySelect: React.FC<{
  ...
  return (
    <div className="relative min-w-[90px] max-w-[120px]">
      <div
        onClick={() => !readOnly && setShowDropdown(!showDropdown)} // Disable dropdown click if readOnly
        className={`flex items-center gap-2 px-2 py-2 border border-gray-300 rounded-lg bg-white cursor-pointer shadow-sm ${readOnly ? 'bg-gray-100 cursor-default' : ''}`} // Apply styling for readOnly
      >
        {selectedCountry && (
          <img
        ...
        {!readOnly && ( // Hide dropdown arrow if readOnly
          <svg
      ...
      {showDropdown && !readOnly && ( // Only show dropdown if not readOnly
...
const Contact = () => {
  ...
  // Effect to prefill form from navigation state or PackageContext
  useEffect(() => {
    ...
    // 1. Process data from navigation state (highest priority)
    ...
    // 2. Process data from selectedPackage/bookingData (lower priority than navigation state)
    ...
    // 3. Process user's own data (lowest priority, but always apply if not already set)
    if (currentUser && userProfile) {
      ...
      // Phone parsing (assumes userProfile.phone is full number with country code)
      if (userProfile.phone) {
        // Try to find if the phone number starts with any known dial code
        ...
        if (matchedCountry) {
          ...
          // Fallback if no country code matches (e.g., just a local number or default to +91)
          if (!combinedPrefillData.countryCode) setAndMark('countryCode', '+91'); // Default if not parsed
          if (!combinedPrefillData.phone) setAndMark('phone', userProfile.phone);
        }
      }
    }
    
    // Apply all collected prefill data and fields
    setFormData(prev => {
    ...
    // Validate all fields after pre-filling to show initial errors if any
    const fieldsToValidateOnLoad = ['firstName', 'lastName', 'email', 'phone', 'destination', 'travelDate', 'travelers', 'message'];
    fieldsToValidateOnLoad.forEach(field => {
        const value = combinedPrefillData[field] || formData[field as keyof typeof formData]; 
        validateField(field, value);
    });

  }, [location.state, selectedPackage, bookingData, validateField, currentUser, userProfile, countriesData]); // Added countriesData to dependencies for phone parsing
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <select
                        ...
                        } ${prefilledFields.has('destination') ? 'bg-gray-100' : ''}`} 
                      >
